...
describe Puppet::Transaction do
  ...
  it "on failure, skips dynamically-generated dependents" do
    ...
    expect(txn.resource_status(purge_dir).skipped).to be_true

    children = catalog.relationship_graph.direct_dependents_of(purge_dir)

    children.each do |child|
      expect(txn.resource_status(child).skipped).to be_true
    end

    expect(Puppet::FileSystem.exist?(File.join(tmp, "foo"))).to be_true
  end
